...
// Mapeo de los IDs de ranking del Backend a Etiquetas Visuales
...
export default function PanelTransacciones() {
  ...
  const [puntosSimulados, setPuntosSimulados] = useState(0);
  const [cargando, setCargando] = useState(true);
  const [error, setError] = useState('');

  // Estados para el simulador de tasación
  ...
  // Cargar historial y ranking desde la base de datos
  useEffect(() => {
  ...
  // HU 12: Tasación automática (igual que en el backend)
  useEffect(() => {
  ...
  if (cargando) {
    return <div className="p-6 text-center">Cargando panel de transacciones...</div>;
  }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen space-y-8 font-sans">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {/* INDICADORES FINANCIEROS (DESDE LA BD) */}
        <div className="grid grid-cols-3 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-100 w-full md:w-auto">
          <div className="text-center px-2">
            ...
            <span className="text-xl font-extrabold text-blue-600">{historial.saldo_actual} <span className="text-xs font-normal text-gray-500">pts</span></span>
          </div>
          ...
          <div className="text-center px-2">
            ...
            <span className="text-xl font-extrabold text-emerald-600">+{historial.puntos_acumulados_historicos} <span className="text-xs font-normal text-gray-500">pts</span></span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* LISTADO DE TRANSACCIONES (DESDE LA BD) */}
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
          <div className="flex justify-between items-center border-b border-gray-100 pb-3">
            ...
            <span className="text-[10px] font-bold text-red-700 bg-red-50 border border-red-200 px-2 py-1 rounded uppercase tracking-wider">
              🔒 Inalterable
            </span>
          </div>
                    ...
                    <tr key={idx} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                          tx.tipo === 'Recibido' ? 'bg-emerald-50 text-emerald-700' : 'bg-orange-50 text-orange-700'
                        }`}>
        ...
        {/* RANKING COMUNITARIO (DESDE LA BD) */}
      ...
      {/* CALCULADORA DE TASACIÓN AUTOMÁTICA EN TIEMPO REAL (HU12) - SOLO SIMULADOR */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            ...
            <select 
              ...
              className="mt-1 block w-full p-2 bg-gray-50 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
            >
          ...
          <div className="bg-blue-50 rounded-lg border border-blue-200 text-center p-3 flex flex-col justify-center">
            <span className="text-[10px] font-bold text-blue-700 uppercase tracking-wider">Valor Calculado</span>
            <div className="flex items-baseline justify-center gap-1">
              <span className="text-2xl font-black text-blue-900">{puntosSimulados}</span>
              <span className="text-xs font-bold text-blue-800">Puntos</span>
            </div>
          </div>
        </div>
        <p className="text-[10px] text-gray-400 mt-3 text-center">Valor regulado ineditable para evitar sobreprecio e inflación comunitaria.</p>
      </div>
    </div>
  );
}